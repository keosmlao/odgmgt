...
export async function rebuildSaleRollup(years = []) {
  ...
  /* ── Seller × month rollup ────────────────────────────────────────────────
     The salesperson is on the bill header, so this one joins ic_trans back on
     doc_no. Sales bills only: 44 sale, 48 sale return — the same set that makes
     up odg_sale_detail. Bills with no sale_code on them are left out; they are
     nobody's assignment.

     ic_trans is cut by doc_date rather than joined blind, because it holds every
     document type the ERP writes, not only sales. The window is the rebuilt
     years plus a month either side, so a bill whose reported month was moved
     across a year boundary is still found. */
  for (const statement of SELLER_TABLE_DDL) await query(statement);
  const icWindow = scoped
    ? `AND t.doc_date >= (SELECT make_date(MIN(y), 1, 1) - INTERVAL '1 month'
                            FROM unnest(%s::int[]) y)
       AND t.doc_date <  (SELECT make_date(MAX(y) + 1, 2, 1) FROM unnest(%s::int[]) y)`
    : "";
  if (scoped) {
    await query(`DELETE FROM ${SELLER_TABLE} WHERE yeardoc = ANY(%s::int[])`, yearParam);
  } else {
    await query(`TRUNCATE ${SELLER_TABLE}`);
  }
  await query(
    `
    INSERT INTO ${SELLER_TABLE}
      (yeardoc, monthdoc, sale_id, bu_code, channel_code, province, amper,
       sum_amount, orders, refreshed_at)
    SELECT
      ${REPORT_YEAR},
      ${REPORT_MONTH},
      btrim(t.sale_code),
