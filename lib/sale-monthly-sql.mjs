 ...
 *
 * Online is decided first and from a different source entirely: no sale is
 * ever tagged 107, so it is recognised by the selling department. See
 * lib/online-channel.mjs.
 *
 * @param {string} docNo bill-number expression, qualified when more than one
 *   table in scope has a doc_no (the rollup joins app_sale_month_override).
 */
export const channelCodeSql = (docNo = "doc_no") => `
  CASE
    WHEN ${isOnlineBillSql(docNo)} THEN '${ONLINE_CHANNEL_CODE}'
...
/** Default form, for callers whose query has one unambiguous doc_no. */
export const CHANNEL_CODE_SQL = channelCodeSql();
