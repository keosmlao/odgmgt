...
let prApprovalReady = false;

/**
 * ຮ່ອງຮອຍການອະນຸມັດໃບຂໍຊື້ — keyed by the ERP document number, the same shape
 * as odg_pm_po_approval, which the PM app created for purchase orders.
 *
 * A requisition is raised in the ERP and this system never sees it until
 * somebody rules on it, so the verdict needs a row of its own — who ruled, when
 * and why, next to the number of the ERP approval document it produced.
 */
export async function ensurePrApprovalTable() {
  if (prApprovalReady) return;
  try {
    await query(`
      CREATE TABLE IF NOT EXISTS public.odg_pm_pr_approval (
    ...
    `);
    // ເລກໃບອະນຸມັດ ERP (WPRA/WPOA) ທີ່ຖືກສ້າງຕອນກົດ — ເອກະສານຢູ່ ERP, ເລກຢູ່ນີ້,
    // ຈຶ່ງຕາມກັນໄດ້ຈາກສອງທາງ. ໃສ່ໃຫ້ຄິວ PO ນຳ; ຕາຕະລາງນັ້ນ PM ສ້າງໄວ້ກ່ອນແລ້ວ.
    await query(`ALTER TABLE public.odg_pm_pr_approval ADD COLUMN IF NOT EXISTS approval_no varchar`);
    await query(`ALTER TABLE public.odg_pm_po_approval ADD COLUMN IF NOT EXISTS approval_no varchar`);
    prApprovalReady = true;
  } catch (error) {
    console.error("ensurePrApprovalTable error:", error);
  }
}
