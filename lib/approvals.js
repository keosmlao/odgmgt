 ...
 *
 * ⚠️ ອະນຸມັດຢູ່ນີ້ = ອະນຸມັດຢູ່ ERP. `ic_trans.approve_status` is the field the
 * ERP's own approval screen sets, and it is what every other report and the
 * purchasing flow reads; a verdict kept only in this system's trail would look
 * approved here and unapproved everywhere else. Only that one column is
 * written, because it is the only one the ERP itself fills — approve_code,
 * user_approve and approve_date sit empty on every document the ERP has
 * approved, and who ruled is kept in the trail instead.
 *
 ...
 */
export async function recordVerdict({ trailTable, docNo, transFlag, action, reason, code }) {
    ...
    if (action === "approve") {
      const erp = await client.query(
        ...
        UPDATE public.ic_trans
         ...
         WHERE doc_no = $1
           AND trans_flag = $2
           AND coalesce(approve_status, 0) = 0
           AND coalesce(last_status, 0) = 0
        `,
        [docNo, transFlag],
      );
      erpUpdated = erp.rowCount;
    }

    await client.query("COMMIT");
    return { ok: true, alreadyHandled: false, erpUpdated };
  } catch (error) {
