import { pool } from "./db";
import { createApprovalDocument, erpApprovalEnabled } from "./erp-approval";
...
/**
 * ບັນທຶກຄຳຕັດສິນ — the verdict, in both places it has to land, or in neither.
 *
 * ⚠️ ອະນຸມັດຢູ່ນີ້ = ອະນຸມັດຢູ່ ERP, ແລະ ການອະນຸມັດຂອງ ERP ແມ່ນ **ເອກະສານ** ບໍ່ແມ່ນ
 * ທຸງ: ໃບອະນຸມັດ WPRA / WPOA ພ້ອມແຖວລາຍການ ຖືກສ້າງ ແລ້ວໃບຕົ້ນທາງຈຶ່ງຖືກໝາຍວ່າ
 * approve_status = 1. ເບິ່ງ lib/erp-approval.js — ມັນເປັນຄົນຂຽນ ແລະ ເປັນຄົນຖື
 * ດ່ານປ້ອງກັນທັງໝົດ.
 *
 * ປະຕິເສດ does NOT touch the ERP: there is no field for it — not_approve_1 is
 * 0 on all 9,220 requisitions and orders — and a rejected document simply stays
 * unapproved, which is already what approve_status = 0 says. The reason lives
 * in the trail, and no approval document is written for a refusal.
 *
 * Both writes share one transaction: a trail row without the ERP flag would
 * leave the document approved on this screen alone.
 */
export async function recordVerdict({ trailTable, docNo, kind, action, reason, code }) {
    ...
    const trail = await client.query(
      ...
      ON CONFLICT (doc_no) DO UPDATE
        SET status = EXCLUDED.status,
            ...
            updated_at = now()
        WHERE lower(coalesce(${trailTable}.status, '')) NOT IN ('approved','rejected','cancelled','canceled','closed')
      `,
      [docNo, action === "approve" ? "approved" : "rejected", code, action === "approve" ? null : reason],
    );

    if (!trail.rowCount) {
      await client.query("ROLLBACK");
      return { ok: false, alreadyHandled: true, erpUpdated: 0 };
    }

    // ອະນຸມັດ = ສ້າງໃບອະນຸມັດໃນ ERP ແລ້ວໝາຍໃບຕົ້ນທາງ. ປະຕິເສດບໍ່ແຕະ ERP.
    let approval = null;
    if (action === "approve" && erpApprovalEnabled()) {
      approval = await createApprovalDocument(client, {
        kind,
        sourceDocNo: docNo,
        approverCode: code,
      });
      if (approval.missing || approval.cancelled || approval.alreadyApproved) {
        await client.query("ROLLBACK");
        return { ok: false, alreadyHandled: true, erp: approval };
      }
      await client.query(
        `UPDATE ${trailTable} SET approval_no = $2, updated_at = now() WHERE doc_no = $1`,
        [docNo, approval.docNo],
      );
    }

    await client.query("COMMIT");
    return { ok: true, alreadyHandled: false, approval };
  } catch (error) {
