...
export async function authLogin(username, password, ip = null) {
  try {
    ...
    // The job title comes from the employee record, not from the access list —
    // the two answer different questions and the card should show the title.
    user.position_name = await readPositionName(user.position_code);
