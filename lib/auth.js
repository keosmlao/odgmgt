...
export function normalizeRole(role, fallback = "sale") {
  if (!role) return fallback;
  return String(role).toLowerCase();
}

export function buildUserPayload(row, fallbackRole = "sale") {
  if (!row) return null;
  return {
    id: row.id ?? null,
    username: row.username ?? null,
    full_name: row.full_name ?? row.name ?? row.fullname ?? null,
    role: normalizeRole(row.role, fallbackRole),
...
export function checkPasswordHash(storedHash, password) {
