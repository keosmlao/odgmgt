      ...
      WITH line AS (
        ...
        FROM (
          SELECT COALESCE(a.employee_code, e.employee_code) AS employee_code,
                 ...
                 CASE
                   WHEN c.pointmap_category = 'Air' AND d.item_name ~ '\\[[CH]\\]\\s*$'
                     THEN d.price + COALESCE(mate.price, 0)
                   ELSE d.price
                 END AS combo_price,
                 -- Whether this component found its other half at all.
                 (c.pointmap_category = 'Air' AND d.item_name ~ '\\[[CH]\\]\\s*$'
                  AND mate.price IS NOT NULL) AS has_mate
          FROM public.odg_sale_detail d
          ${OVERRIDE_JOIN}
          -- The other half of the set, and its price. The two halves are
