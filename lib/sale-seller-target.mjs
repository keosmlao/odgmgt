 ...
 * Managers own no plan row in the channel they run — their board figure there is
 * the roll-up of the sellers under them, so counting it would count the plan
 * twice — but a channel they merely sell they claim like anyone else.
 * See app/api/sales-assignments for the roll-up the board displays.
 */
import { claimableChannelSql, managesChannelSql } from "./sales-board-roles.mjs";
  ...
  return `
      ...
      claim AS (
        ...
        JOIN public.odg_sales_target st
         ...
         -- A manager owns no plan row IN THE CHANNEL THEY RUN: their number
         -- there is the sum of what their sellers carry, so counting it would
         -- count the plan twice. A channel they merely sell is theirs to claim
         -- like anyone else's. See app/api/sales-assignments for the roll-up.
