...
/**
 * The job title a person actually holds, from public.odg_position.
 *
 * Not the same thing as `role`: role is this app's permission level, set on the
 * access list, and showing it on someone's name card said "CEO" under a name
 * whose position_code is 11 — ຜູ້ຈັດການ. The two are allowed to differ, and the
 * card should read the way HR files them.
 *
 * The master table belongs to another system, so a missing table or column is
 * a blank title, never a failed login.
 */
export async function readPositionName(code) {
  const value = String(code ?? "").trim();
  if (!value) return null;
  try {
    const row = await one(
      `SELECT COALESCE(NULLIF(btrim(position_name_lo), ''), NULLIF(btrim(position_name_en), '')) AS name
       FROM public.odg_position WHERE btrim(position_code) = %s`,
      [value],
    );
...
export function buildEmployeeUser(row, schema, fallbackUsername) {
  ...
  return {
    ...
    // The employee's own job title, kept apart from `role`, which is a
    // permission level this app assigns. readPositionName resolves the name.
