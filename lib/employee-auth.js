...
/**
 * Compares the password against what odg_employee stores, as plain text.
 * Hashing was removed on request: any stored value that still looks like a
 * hash can never match, so it is reported separately instead of as a wrong
 * password.
 */
export function verifyEmployeePassword(stored, password) {
  ...
  if (/^(scrypt|pbkdf2|argon2)[$:]/i.test(value) || /^\$(2[aby]?|6|5|1)\$/.test(value)) {
