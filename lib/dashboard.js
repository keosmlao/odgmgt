...
async function computeDashboardPayload(year, bu, channel, province, month = null) {
  ...
  const [
    ...
    rows(
      `SELECT item_code AS code, item_name AS name, COALESCE(SUM(sum_amount), 0)::float AS revenue
       FROM ${SALE_DETAIL_REPORTED} WHERE ${detailWhere} GROUP BY code, name ORDER BY revenue DESC LIMIT 5`,
      detailParams,
    ),
    rows(
      `SELECT item_code AS code, item_name AS name,
       ...
       FROM ${SALE_DETAIL_REPORTED} WHERE ${detailWhere} GROUP BY code, name ORDER BY gp DESC NULLS LAST, revenue DESC LIMIT 5`,
      detailParams,
    ),
    rows(
  ...
  const [teamActualRows, teamTargetRows, buProfitRows, subgroupRows, topCustomerRows] = await Promise.all([
    ...
    rows(
      `SELECT COALESCE(NULLIF(btrim(bu_code), ''), '-') AS bu,
        COALESCE(SUM(sum_amount), 0)::float AS revenue,
  ...
  if (brandCol) {
    const brandRows = await rows(
      `SELECT COALESCE(NULLIF(btrim(bu_code), ''), '-') AS bu,
        COALESCE(NULLIF(${brandCol}, ''), 'UNKNOWN') AS brand,
        COALESCE(SUM(sum_amount), 0)::float AS revenue,
        ${costThbExpr} AS cost_thb, ${costLaoExpr} AS cost_lao
       FROM ${SALE_DETAIL_REPORTED} WHERE ${detailWhere}
  ...
  const arBuLookup = `LEFT JOIN (SELECT DISTINCT doc_no, bu_code, bu_name FROM ${SALE_DETAIL_REPORTED} WHERE doc_no IS NOT NULL) sd ON sd.doc_no = a.doc_no`;
  ...
  const result = {
    ...
    // Counted in the same scan that produces the YTD figure. It was computed
    // and then never published, so every "customers" and "orders" reading on
    // the dashboard was a hard zero.
    counts,
  };
