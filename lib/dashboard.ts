...
import { parseIntSafe, safeDiv, monthName, CHANNEL_EXPR } from "./helpers";
import { buildFilters, channelMap } from "./filters";
import { ensureSalesAssignmentTable } from "./migrations";
import { getSaleDetailSchema } from "./sale-detail-schema";
import type { ChannelMapEntry, FilterValue } from "./types";

// ── Dashboard result cache (TTL 5 min) ──
interface DashboardPayload {
  kpi: KpiSummary;
  trend: TrendPoint[];
  structure: {
    buChannel: QueryResultRow[];
    buProfit: BuProfit[];
  ...
  payment: { cash: number; credit: number };
}
...
const DASHBOARD_CACHE_TTL = 300_000; // 5 minutes
...
interface KpiSummary {
  ...
  this_month_last_year: number;
  last_month_actual: number;
  last_month_target: number;
  cash_ytd_actual: number;
  credit_ytd_actual: number;
...
type ArResult = [QueryResultRow[], QueryResultRow[], QueryResultRow | null];
type StockResult = [
  QueryResultRow | null,
  QueryResultRow[],
  QueryResultRow[],
...
export function buildActionsFromKpi(kpi: KpiSummary): DashboardAction[] {
...
function buildHistoricalDetailFilters(
  year: number,
  month: number,
  bu: FilterValue,
  channel: FilterValue,
  province: FilterValue,
): { selectedYm: number; detailWhere: string; detailParams: unknown[] } {
  const selectedYm = year * 12 + month;
  const detailWhere = ["(yeardoc * 12 + monthdoc) <= %s"];
  const detailParams: unknown[] = [selectedYm];
...
export async function buildDashboardPayload(
  ...
  channel: FilterValue,
  province: FilterValue,
  month: FilterValue = null,
): Promise<DashboardPayload> {
  const cacheKey = `${year}|${bu}|${channel}|${province}|${month}`;
  const cached = getCachedDashboard(cacheKey);
  if (cached) return cached;
  const now = new Date();
  const yearVal = parseIntSafe(year, now.getFullYear()) as number;
  const currentMonthVal = now.getMonth() + 1;
  const selectedMonthVal = month && String(month) !== "ALL"
    ? (parseIntSafe(month, currentMonthVal) as number)
    : currentMonthVal;
  let lastMonthVal = currentMonthVal - 1;
  ...
  const cashTerms = ["cash", "สด", "เงินสด", "cs", "cod"];
  const cashLike = ["%cash%", "%สด%", "%เงินสด%", "%cod%"];
  const creditTerms = ["credit", "เชื่อ", "ติดหนี้", "cr"];
  const creditLike = ["%credit%", "%เชื่อ%", "%ติดหนี้%", "%cr%"];
  ...
  const [
    ...
    one(
      `SELECT
        ...
        COALESCE(SUM(CASE WHEN lower(COALESCE(saletype, '')) = ANY(%s) OR lower(COALESCE(saletype, '')) LIKE ANY(%s) THEN sum_amount ELSE 0 END), 0)::float AS cash_total,
        COALESCE(SUM(CASE WHEN lower(COALESCE(saletype, '')) = ANY(%s) OR lower(COALESCE(saletype, '')) LIKE ANY(%s) THEN sum_amount ELSE 0 END), 0)::float AS credit_total
       FROM public.odg_sale_detail WHERE ${detailWhere}`,
      [currentMonthVal, cashTerms, cashLike, creditTerms, creditLike, ...detailParams],
    ),
    one(
      `SELECT COALESCE(SUM(sum_amount), 0)::float AS total
       FROM public.odg_sale_detail WHERE ${detailWhere.replace("yeardoc = %s", "yeardoc = %s")} AND monthdoc = %s`,
      [yearVal - 1, ...detailParams.slice(1), currentMonthVal],
    ),
    one(
      `SELECT COALESCE(SUM(sum_amount), 0)::float AS total
       FROM public.odg_sale_detail WHERE ${detailWhere.replace("yeardoc = %s", "yeardoc = %s")} AND monthdoc = %s`,
      [lastMonthYear, ...detailParams.slice(1), lastMonthVal],
    ),
    one(
      `SELECT COALESCE(SUM(sum_amount), 0)::float AS total
       FROM public.odg_sale_detail WHERE ${detailWhere.replace("yeardoc = %s", "yeardoc = %s")}`,
      [yearVal - 1, ...detailParams.slice(1)],
    ),
    // Combined: monthly actual + cash/credit breakdown in 1 scan
    rows(
      `SELECT CAST(monthdoc AS INT) AS month,
        COALESCE(SUM(sum_amount), 0)::float AS actual,
        COALESCE(SUM(CASE WHEN lower(COALESCE(saletype, '')) = ANY(%s) OR lower(COALESCE(saletype, '')) LIKE ANY(%s) THEN sum_amount ELSE 0 END), 0)::float AS cash,
        COALESCE(SUM(CASE WHEN lower(COALESCE(saletype, '')) = ANY(%s) OR lower(COALESCE(saletype, '')) LIKE ANY(%s) THEN sum_amount ELSE 0 END), 0)::float AS credit
       FROM public.odg_sale_detail WHERE ${detailWhere} GROUP BY CAST(monthdoc AS INT)`,
      [cashTerms, cashLike, creditTerms, creditLike, ...detailParams],
    ),
    rows(
      `SELECT monthdoc AS month, COALESCE(SUM(sum_amount), 0)::float AS actual
       FROM public.odg_sale_detail WHERE ${detailWhere.replace("yeardoc = %s", "yeardoc = %s")} GROUP BY monthdoc`,
      [yearVal - 1, ...detailParams.slice(1)],
    ),
    rows(
      `SELECT monthdoc AS month, COALESCE(SUM(sum_amount), 0)::float AS actual
       FROM public.odg_sale_detail WHERE ${detailWhere.replace("yeardoc = %s", "yeardoc = %s")} GROUP BY monthdoc`,
      [yearVal - 2, ...detailParams.slice(1)],
    ),
    ...
    rows(
      `SELECT bu_code AS bu,
        ${CHANNEL_EXPR} AS channel,
        COALESCE(SUM(sum_amount), 0)::float AS amount
       FROM public.odg_sale_detail WHERE ${detailWhere} GROUP BY bu, channel`,
      detailParams,
    ),
    rows(
      `SELECT province AS province_code,
        COALESCE(NULLIF(province_name, ''), province) AS label,
        COALESCE(SUM(sum_amount), 0)::float AS actual
       FROM public.odg_sale_detail WHERE ${detailWhere} GROUP BY province, label`,
      detailParams,
    ),
    rows(
    ...
    rows(
      `SELECT customer_code, COUNT(*) AS cnt
       FROM public.odg_sale_detail WHERE ${detailWhere} GROUP BY customer_code`,
      detailParams,
    ),
    one(
      `WITH scoped AS (
      ...
      previous_purchase AS (
        ...
        LEFT JOIN scoped s
          ON s.customer_code = cc.customer_code
         AND s.ym < %s
  ...
  const targetMap: Record<number, number> = Object.fromEntries(
    targetRowsResult.map((row) => [Number(row.month), Number(row.target || 0)]),
  );
  const ytdTarget = targetRowsResult.reduce((s, r) => s + Number(r.target || 0), 0);
  const thisMonthTarget = Number(targetMap[currentMonthVal] || 0);
  ...
  const cashCreditMap: Record<number, { cash: number; credit: number }> = {};
  ...
  const thisMonthCash = Number(cashCreditMap[currentMonthVal]?.cash || 0);
  const thisMonthCredit = Number(cashCreditMap[currentMonthVal]?.credit || 0);
  ...
  const lastYearMap: Record<number, number> = Object.fromEntries(
    lastYearRows.map((row) => [Number(row.month), Number(row.actual || 0)]),
  );
  const last2YearMap: Record<number, number> = Object.fromEntries(
    last2YearRows.map((row) => [Number(row.month), Number(row.actual || 0)]),
  );
  const buChannel = buChannelResult;

  const counts = { customers: Number(kpiComboRow?.customers || 0), orders: Number(kpiComboRow?.orders || 0) };
  const disc = { discount_total: Number(kpiComboRow?.discount_total || 0), revenue_total: Number(kpiComboRow?.revenue_total || 0) };

  const provinceTargetMap: Record<string, number> = Object.fromEntries(
    provinceTargetRows.map((row) => [String(row.province_code), Number(row.target || 0)]),
  );
  const provinceRows: ProvinceRow[] = provinceActualRows.map((row) => {
    const code = String(row.province_code);
  ...
  const dynamicChannelMap = (arGroupRows || []).reduce<Record<string, ChannelMapEntry>>((accumulator, row) => {
  ...
  let channelParamsDetail: unknown[] = [];
  ...
  let channelParamsTarget: unknown[] = [];
    ...
    rows(
      `SELECT a.sale_id,
       ...
       LEFT JOIN public.odg_sale_detail d
         ON d.yeardoc = %s AND d.monthdoc = a.month AND d.bu_code = a.bu_code
      ...
      [yearVal, ...channelParamsDetail, ...assignParams],
    ),
    rows(
      `SELECT a.sale_id,
        COALESCE(NULLIF(a.sale_name, ''), a.sale_id) AS sale_name,
  ...
  const buProfit: BuProfit[] = buProfitRows
    .map((row) => {
  ...
  const groupMap: Record<string, GroupProfit> = {};
  for (const row of subgroupRows) {
    ...
    groupMap[groupName].revenue += revenue;
    groupMap[groupName].cost_thb += costThb;
    groupMap[groupName].cost_lao += costLao;
    groupMap[groupName].profit_thb += profitThb;
    groupMap[groupName].profit_lao += profitLao;
    groupMap[groupName].subgroups.push({
  ...
  const sortedGroupProfit = Object.values(groupMap).sort(
    (left, right) => right.profit_thb - left.profit_thb,
  );
  const totalGroupProfit = sortedGroupProfit.reduce(
    (sum, item) => sum + Number(item.profit_thb || 0),
    0,
  );
  const groupProfit = sortedGroupProfit.slice(0, 10).map((item) => ({
    ...item,
  ...
  for (let monthIndex = 1; monthIndex <= 12; monthIndex += 1) {
    const cashCredit = cashCreditMap[monthIndex] || { cash: 0, credit: 0 };
    trend.push({
      month: monthIndex,
  ...
  const qualified = Math.round(leads * 0.7);
  const proposal = Math.round(leads * 0.45);
  const negotiation = Math.round(leads * 0.22);
  const won = orders;
  const conversionPct = leads ? safeDiv(won, leads) * 100 : 0;
  const avgDeal = won ? safeDiv(totalActual, won) : 0;
  ...
  const newCustomerPct = Math.max(0, 100 - repeatPct);
  ...
  const arJoin = `LEFT JOIN (SELECT DISTINCT doc_no, bu_code, bu_name FROM public.odg_sale_detail WHERE doc_no IS NOT NULL) sd ON sd.doc_no = a.doc_no`;
  ...
  const [arResults, stockResults] = await Promise.all([
    ...
    ]).catch((error): ArResult => { console.error("ar_aging error:", error); return [[], [], null]; }),
    Promise.all([
      ...
      rows(`SELECT COALESCE(NULLIF(dim_8, ''), 'UNKNOWN') AS warehouse,
      ...
      rows(`SELECT COALESCE(NULLIF(mg.name_1, ''), ic.group_main, 'UNKNOWN') AS group_main_name,
    ...
    ]).catch((error): StockResult => { console.error("stockSummary error:", error); return [null, [], [], [], []]; }),
  ]);
  ...
  const result: DashboardPayload = {
    ...
    product: {
      ...
      groupProfit,
      quality: {
        discountRatePct: disc.revenue_total
          ? safeDiv(disc.discount_total, disc.revenue_total) * 100
          : 0,
        repeatPct,
        newCustomerPct,
        reactiveCustomers,
  ...
  dashboardCache.set(cacheKey, { ts: Date.now(), data: result });
  return result;
}
