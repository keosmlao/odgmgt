...
/**
 * Cash and bank movement, from the cash book (public.cb_trans).
 *
 * cb_trans records MOVEMENT, not balances: each row splits its value across
 * cash / transfer / cheque / card / deposit columns. Closing balances live in
 * the GL (gl_journal_detail against the cash and bank accounts) and are NOT
 * derived here — showing a running total off this table alone would overstate
 * it, since the same money can appear on both a receipt and a deposit row.
 *
 * book_code is empty on every row, so it cannot separate accounts; trans_flag
 * is what distinguishes document types (44 = sale bill, 48 = credit note).
 */
export async function loadCashBank({ days = 90 } = {}) {
  const window = Math.max(1, Math.min(365, Number(days) || 90));

  const [totals, daily, monthly, byFlag, recent] = await Promise.all([
    one(
      `SELECT COALESCE(SUM(cash_amount), 0)::float AS cash,
              COALESCE(SUM(tranfer_amount), 0)::float AS transfer,
              COALESCE(SUM(chq_amount), 0)::float AS cheque,
              COALESCE(SUM(card_amount), 0)::float AS card,
              COALESCE(SUM(deposit_amount), 0)::float AS deposit,
              COALESCE(SUM(total_net_amount), 0)::float AS net,
              COUNT(*)::int AS docs
       FROM public.cb_trans WHERE doc_date >= current_date - %s::int`,
      [window],
    ),
    rows(
      `SELECT doc_date::text AS day,
              COALESCE(SUM(cash_amount), 0)::float AS cash,
              COALESCE(SUM(tranfer_amount), 0)::float AS transfer,
              COALESCE(SUM(chq_amount), 0)::float AS cheque,
              COALESCE(SUM(deposit_amount), 0)::float AS deposit,
              COUNT(*)::int AS docs
       FROM public.cb_trans WHERE doc_date >= current_date - 29
    ...
    rows(
      `SELECT to_char(doc_date, 'YYYY-MM') AS month,
              ...
              COALESCE(SUM(total_net_amount), 0)::float AS net,
              COUNT(*)::int AS docs
       FROM public.cb_trans WHERE doc_date >= (current_date - interval '12 months')
       GROUP BY 1 ORDER BY 1 DESC`,
    ),
    rows(
      `SELECT trans_flag::int AS trans_flag, COUNT(*)::int AS docs,
              COALESCE(SUM(total_net_amount), 0)::float AS net
       FROM public.cb_trans WHERE doc_date >= current_date - %s::int
    ...
    rows(
      `SELECT doc_no, doc_date::text AS doc_date, description, remark,
              COALESCE(cash_amount, 0)::float AS cash,
              COALESCE(tranfer_amount, 0)::float AS transfer,
              COALESCE(chq_amount, 0)::float AS cheque,
              COALESCE(deposit_amount, 0)::float AS deposit,
              COALESCE(total_net_amount, 0)::float AS net
       FROM public.cb_trans
       WHERE doc_date >= current_date - %s::int
       ORDER BY doc_date DESC, doc_no DESC LIMIT 50`,
      [window],
    ),
  ]);

  return { days: window, totals, daily, monthly, byFlag, recent };
}
