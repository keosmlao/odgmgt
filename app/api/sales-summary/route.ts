...
export async function GET(request: NextRequest) {
  try {
    ...
    const now = new Date();
    const yearVal = parseIntSafe(year, now.getFullYear()) ?? now.getFullYear();
    const monthVal = parseIntSafe(month, now.getMonth() + 1) ?? now.getMonth() + 1;
    ...
    let chClauseD = "";
    let chParamsD: unknown[] = [];
    let chClauseT = "";
    let chParamsT: unknown[] = [];
    ...
    const chLabel = (raw: unknown): string => {
    ...
    const sumRows = (items: SummaryRow[]): SummaryRow => {
    ...
    // By area (province)
    const areaTargetObj: Record<string, number> = Object.fromEntries(areaTargetRowsRes.map((r) => [String(r.province_code), Number(r.target || 0)]));
    const areaYtdActObj: Record<string, number> = Object.fromEntries(areaYtdActualRowsRes.map((r) => [String(r.province_code), Number(r.actual || 0)]));
    const areaYtdTgtObj: Record<string, number> = Object.fromEntries(areaYtdTargetRowsRes.map((r) => [String(r.province_code), Number(r.target || 0)]));
    ...
    // By sale
    const saM: Record<string, QueryResultRow> = Object.fromEntries(saleActualRowsRes.map((r) => [String(r.sale_id), r]));
    const stM: Record<string, QueryResultRow> = Object.fromEntries(saleTargetRowsRes.map((r) => [String(r.sale_id), r]));
    const syaM: Record<string, QueryResultRow> = Object.fromEntries(saleYtdActualRowsRes.map((r) => [String(r.sale_id), r]));
    const sytM: Record<string, QueryResultRow> = Object.fromEntries(saleYtdTargetRowsRes.map((r) => [String(r.sale_id), r]));
