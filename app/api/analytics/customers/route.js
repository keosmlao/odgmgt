...
import { analyticsCacheKey, loadCustomersAnalytics } from "@/lib/analytics";
...
export async function GET(request) {
  try {
    ...
    const data = await swrCache(
      analyticsCacheKey("customers", { year, bu, channel, province }),
      {
        ttl: 300_000,
        staleTtl: 24 * 3_600_000,
        bypass: sp.get("nocache") === "1",
      },
      () => loadCustomersAnalytics({ year, bu, channel, province }),
    );
