...
export async function GET(request: NextRequest) {
  try {
    ...
    let lastMonthYear = year as number;
    if (lastMonth <= 0) { lastMonth = 12; lastMonthYear = (year as number) - 1; }
    ...
    const dayOfYear = Math.floor((now.getTime() - new Date(year as number, 0, 1).getTime()) / 86400000) + 1;
    ...
    const lastMCust = Number(churnRows?.last_month_customers || 0);
    const churnedCount = Number(churnRows?.churned || 0);
    const newAcquired = Number(churnRows?.new_acquired || 0);
