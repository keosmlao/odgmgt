...
): { detailWhere: string; detailParams: unknown[]; selectedYm: number } {
  const yearValue = parseIntSafe(year, new Date().getFullYear()) as number;
  const monthValue = parseIntSafe(month, new Date().getMonth() + 1) as number;
