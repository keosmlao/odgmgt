...
import type { ChannelMapEntry } from "@/lib/types";

const cacheMap = new Map<string, { ts: number; data: unknown }>();
const TTL = 300_000;
...
function summarizeSegment(rowsList: { revenue: number }[]): { count: number; revenue: number } {
  const totals = rowsList.reduce<{ count: number; revenue: number }>(
    (accumulator, row) => {
      accumulator.count += 1;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const now = new Date();
    const year = parseIntSafe(sp.get("year"), now.getFullYear()) ?? now.getFullYear();
    ...
    const dayRank: Record<string, number> = { MON: 1, TUE: 2, WED: 3, THU: 4, FRI: 5, SAT: 6, SUN: 7 };
    const dayTimeCells = (heatmapRows || []).map((row) => ({
      day: row.day_name as string,
      hour: row.hour_slot as string,
